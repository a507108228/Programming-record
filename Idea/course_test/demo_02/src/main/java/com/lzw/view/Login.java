...
public class Login{
    ...
    public void LoginGui(){
        ...
        ImageIcon img = new ImageIcon("D:\\Teaching\\Idea\\course_test\\demo_02\\src\\main\\resources\\登录背景.jpg"); //传入背景图片路径
            ...
            public void actionPerformed(ActionEvent e){
                ...
                if (state == "2") {
                    try {
                        //执行sql语句，进行数据库添加
                        boolean flag = l
                                .Query(user, "select * from users where username=? and userpwd=? and flag=" + state);
                        if (flag) {
                            //文本提示框
